...
本リポジトリは、バックエンドAPI、Webアプリフロントエンド、Garminスマートウォッチ向けアプリ（データフィールド版・単体アプリ版）を内包するモノレポ構成となっています。
...
├── yamakage-datafield/     # ⌚️ Garmin Connect IQ アプリ (データフィールド版 / Monkey C)
├── yamakage-watch-app/     # ⌚️ Garmin Connect IQ アプリ (WatchApp単体アプリ版 / Monkey C)
...
Garmin製スマートウォッチ向けの Connect IQ ウォッチアプリ（単体アプリ）です。
アクティビティ中以外でもユーザーの任意のタイミングで起動し、リッチなグラフィックで周囲の地形と日照状況を視覚的に把握できます。

- **スタック:** Monkey C

- **機能:** ボタン起点のオンデマンドなGPS取得とAPI通信。向いている方角の山のシルエットを描画する「パノラマビュー」、360度の地形と太陽軌道を俯瞰する「SkyPlot（天空図）」など、高度なグラフィック描画機能。
