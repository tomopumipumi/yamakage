...
module MonkeyHooks {
    ...
    // ========================================================
    // Custom Type Interface Template
    // ========================================================
    /* 
    class [Type]Context {
        ...
        function get() as [Type]? { return _cx.get() as [Type]?; }
        function req() as [Type] { 
            var val = _cx.get();
            if (val == null) { throw new Lang.InvalidValueException("MonkeyHooks: [Type] req() failed."); }
            return val as [Type];
        }
        function set(val as [Type]?) as Void { _cx.set(val); }
        function setSilent(val as String?) as Void { _cx.setSilent(val); }
        function init(val as [Type]) as [Type]Context { _cx.init(val); return self; }
    ...
    function use[Type](key as Object) as [Type]Context {
        return new [Type]Context(MonkeyHooks.useArena(key));
    }
    */
}
