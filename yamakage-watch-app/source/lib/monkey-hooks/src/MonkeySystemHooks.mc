    ...
    module SharedTimer {
        ...
        class _TimerDelegate {
            function onTick() as Void {
                MonkeyHooks.SharedTimer.triggerTick();
            }
        }
    ...
    module LocationHook {
        ...
        class _LocationDelegate {
            function onLocation(info as Position.Info) as Void {
                MonkeyHooks.LocationHook.triggerLocation(info);
            }
        }
        ...
        function triggerLocation(info as Position.Info) as Void {
            if (_listeners != null) {
                for (var i = 0; i < _listeners.size(); i++) {
                    var listener = _listeners[i] as Lang.Method;
