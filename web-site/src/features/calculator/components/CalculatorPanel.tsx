...
export const CalculatorPanel: React.FC = () => {
  const { calculate, setTurnstileToken, turnstileToken, sunsetTime, isLoading } =
    useCalculatorStore();

  const turnstileRef = useRef<TurnstileInstance>(null);
  const { isMobile, isMinimized, setIsMinimized, isDragging, currentTranslateY, handlers } =
    usePanelDrag();
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-5 md:space-y-6">
        <CalculatorForm turnstileRef={turnstileRef} onCalculate={handleCalculate} />
        <CalculatorResults />
      </div>
    </div>
  );
