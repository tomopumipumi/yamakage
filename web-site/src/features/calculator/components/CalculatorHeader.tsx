...
import { Watch, Share2, Info, Settings } from 'lucide-react';
  ...
  return (
    <div className="shrink-0 bg-gradient-to-r from-orange-600 to-purple-700 p-5 md:p-6 flex items-center justify-between relative">
      ...
      <div className="grid grid-cols-2 gap-1.5 md:gap-2 relative z-20">
        <IconButton Icon={Watch} onClick={() => setSmartwatchOpen(true)} ariaLabel={t('smartwatch.menu_title')} />
        <IconButton Icon={Share2} onClick={() => setShareOpen(true)} ariaLabel={t('share.menu_title')} />
        <IconButton Icon={Info} onClick={() => setAboutOpen(true)} ariaLabel={t('about.menu_title')} />
        <IconButton Icon={Settings} onClick={() => setSettingsOpen(true)} ariaLabel={t('settings')} />
      </div>
    </div>
  );
};